...
DEFINE_HOOK(6A9348, CameoClass_GetTip_FixLength, 9)
{
	...
	if(HideObjectName) {
		...
		_snwprintf_s(SidebarClass::TooltipBuffer(), 0x42, 0x42 - 1, Format, Cost);
	} else {
		const wchar_t * UIName = Object->UIName;
		const wchar_t * Format = StringTable::LoadString("TXT_MONEY_FORMAT_2");
		_snwprintf_s(SidebarClass::TooltipBuffer, 0x42, 0x42 - 1, Format, UIName, Cost);
	}
	SidebarClass::TooltipBuffer[0x42 - 1] = 0;
