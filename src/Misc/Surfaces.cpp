...
{
	void* Blitter;
	if (WWMouseClass::Instance->Image->HasCompression(WWMouseClass::Instance->ImageFrameIndex))
		Blitter = FileSystem::MOUSE_PAL->SelectRLEBlitter(BlitterFlags::None);
	else
		Blitter = FileSystem::MOUSE_PAL->SelectPlainBlitter(BlitterFlags::None);
...
//			Surface->BlitPart(&DestRect, DSurface::Primary, &ClipRect, 0, 1);
			...
			if(WORD * buffer = reinterpret_cast<WORD *>(Surface->Lock(0, 0))) {
				...
				int pitch = Surface->SurfaceDesc->lPitch;
